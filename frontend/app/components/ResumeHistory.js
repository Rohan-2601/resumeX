...
export default function ResumeHistory() {
  ...
  const fetchVersions = async () => {
    ...
    try {
      ...
      const resumeContainer = resumeRes.data.resume;
            ...
            return (
              <li key={v._id} style={{ 
                ...
                <div>
                  ...
                  {isCurrent && <span style={{ marginLeft: "0.5rem", color: "blue", fontSize: "0.8rem", fontWeight: "bold" }}>(Current active)</span>}
                  <div style={{ fontSize: "0.85rem", color: "#666", marginTop: "0.25rem" }}>
                    Uploaded: {new Date(v.createdAt).toLocaleString()}
                  </div>
                  <div style={{ marginTop: "0.5rem" }}>
                    <a href={v.fileUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#0066cc", textDecoration: "none" }}>
                      View PDF
                    </a>
                  </div>
                </div>
                ...
                {!isCurrent && (
                  <button 
                    ...
                    style={{ padding: "0.5rem 1rem", cursor: rollingBack ? "not-allowed" : "pointer" }}
                  >
                    Rollback to this
                  </button>
                )}
