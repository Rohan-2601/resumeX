...
export const createLink = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { resumeId, slug, label } = req.body;

    if (!resumeId || !slug) {
      return res
        .status(400)
        .json({ message: "resumeId and slug are required" });
    }
    ...
    if (!resume || resume.userId.toString() !== userId) {
      return res
        .status(403)
        .json({ message: "Not authorized to link this resume" });
    }
    ...
    if (existing) {
      return res
        .status(400)
        .json({ message: "This custom URL is already in use by you" });
    }
